...
func Test_DisplayRequest(t *testing.T) {
	file := "/tmp/shamironsalt.Test_DisplayRequest." + strconv.Itoa(int(time.Now().Unix()))
	...
	_, _, _, _, _, _, _, _, _ = pub, priv, pub1, priv1, pub2, priv2, priv3, pub4, priv4
	...
	ResponseFile := file + ".responsetoshare"
